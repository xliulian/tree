...
var Tree = /*#__PURE__*/function (_React$Component) {
  ...
  function Tree() {
    ...
    _this.onNodeDragEnter = function (event, node) {
      // console.log('onNodeDragEnter', event, node)
    ...
    _this.onNodeDragOver = function (event, node) {
      ...
      if (dragNode.props.eventKey === dropTargetKey && dropLevelOffset === 0) {
        ...
        // console.log('onNodeDragOver', event, node, this.state.dragOverNodeKey, this.currentMouseOverDroppableNodeKey, node.props.eventKey)
        _this.setState({
          dropPosition: dropPosition,
          dropLevelOffset: dropLevelOffset,
          dropTargetKey: dropTargetKey,
    ...
    _this.onNodeDragLeave = function (event, node) {
      // console.log('onNodeDragLeave', event, node, this.state.dragOverNodeKey, this.currentMouseOverDroppableNodeKey, node.props.eventKey, this.currentMouseOverDroppableNodeKey === node.props.eventKey && !event.currentTarget.contains(event.relatedTarget as Node), this.dragEventQueue.length)
    ...
    _this.dragEventQueue = [];
    _this.dragEventTimer = null;

    _this.queueDragEvent = function (handler, event, node) {
      var lastEvent = _this.dragEventQueue[_this.dragEventQueue.length - 1];

      if (lastEvent) {
        if (handler === _this.onNodeDragOver && lastEvent[0] === handler && lastEvent[2] === node) {
          lastEvent[1] = event;
          return;
        }
      }

      if (handler === _this.onNodeDragLeave && !event.currentTarget.contains(event.relatedTarget)) {
        _this.dragEventQueue = _this.dragEventQueue.filter(function (e) {
          return e[2] !== node;
        });

        if (!_this.dragEventQueue.length) {
          return handler(event, node);
        }
      }

      _this.dragEventQueue.push([handler, event, node]);

      if (!_this.dragEventTimer) {
        _this.dragEventTimer = setTimeout(function () {
          return _this.processDragEvents();
        }, 100);
      }
    };

    _this.processDragEvents = function () {
      if (_this.dragEventTimer) {
        clearTimeout(_this.dragEventTimer);
        _this.dragEventTimer = null;
      }

      while (_this.dragEventQueue.length > 0) {
        var _this$dragEventQueue$ = (0, _slicedToArray2.default)(_this.dragEventQueue.splice(0, 1)[0], 3),
            handler = _this$dragEventQueue$[0],
            event = _this$dragEventQueue$[1],
            node = _this$dragEventQueue$[2];

        if (!event.currentTarget) {
          event.currentTarget = event.target.classList.contains("".concat(_this.props.prefixCls, "-treenode")) ? event.target : event.target.closest(".".concat(_this.props.prefixCls, "-treenode"));
        }
    ...
    _this.onNodeDragEnd = function (event, node) {
      ...
      onDragEnd === null || onDragEnd === void 0 ? void 0 : onDragEnd({
        event: event,
        node: node && (0, _treeUtil.convertNodePropsToEventData)(node.props)
      });
      _this.dragNode = null;
    };
    ...
    value: function render() {
      ...
      return /*#__PURE__*/React.createElement(_contextTypes.TreeContext.Provider, {
        value: {
          ...
          onNodeDragEnter: function onNodeDragEnter(e, node) {
            return _this2.queueDragEvent(_this2.onNodeDragEnter, e, node);
          },
          onNodeDragOver: function onNodeDragOver(e, node) {
            return _this2.queueDragEvent(_this2.onNodeDragOver, e, node);
          },
          onNodeDragLeave: function onNodeDragLeave(e, node) {
            return _this2.queueDragEvent(_this2.onNodeDragLeave, e, node);
          },
